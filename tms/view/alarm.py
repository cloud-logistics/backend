    ...
    for s in sensors:
        try:
            ...
            if s.temperature > fish_type.temperature_max:
                msg = '当前温度：' + s.temperature + '，超过最高温度阈值：' + fish_type.temperature_max + ';'
            elif s.temperature < fish_type.temperature_min:
                msg = '当前温度：' + s.temperature + '，低于最低温度阈值：' + fish_type.temperature_min + ';'

            if s.salinity > fish_type.salinity_max:
                msg += '当前盐度：' + s.salinity + '，超过最高盐度阈值：' + fish_type.salinity_max + ';'
            elif s.salinity < fish_type.salinity_min:
                msg += '当前盐度：' + s.salinity + '，低于最低盐度阈值：' + fish_type.salinity_min + ';'

            if s.ph > fish_type.ph_max:
                msg += '当前PH值：' + s.ph + '，超过最高PH阈值：' + fish_type.ph_max + ';'
            elif s.ph < fish_type.ph_min:
                msg += '当前PH值：' + s.ph + '，低于最低PH阈值：' + fish_type.ph_min + ';'

            if s.dissolved_oxygen > fish_type.dissolved_oxygen_max:
                msg += '当前水溶氧：' + s.dissolved_oxygen + '，超过最高水溶氧阈值：' + fish_type.dissolved_oxygen_max + ';'
            elif s.dissolved_oxygen < fish_type.dissolved_oxygen_min:
                msg += '当前水溶氧：' + s.dissolved_oxygen + '，低于最低水溶氧阈值：' + fish_type.dissolved_oxygen_min + ';'

            notify_alarm(msg, user_id)
            save_alarm(msg, user_id, s.deviceid, fishing.qr_id)
