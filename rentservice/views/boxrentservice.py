...
from rentservice.models import RentLeaseInfo, RentalServiceAdmin, EnterpriseUser, RentalAdminOperationRecords
...
def rent_boxes_order(request):
    ...
    try:
        ...
        return JsonResponse(retcode(errcode("9999", '租赁操作员id不能为空'), "9999", '租赁操作员id不能为空'), safe=True, status=status.HTTP_400_BAD_REQUEST)
    try:
        rent_user_id = data['rent_user_id']
    except Exception:
        return JsonResponse(retcode(errcode("9999", '租赁用户id不能为空'), "9999", '租赁用户id不能为空'), safe=True, status=status.HTTP_400_BAD_REQUEST)
    try:
        site_id = data['site_id']
    except Exception:
        return JsonResponse(retcode(errcode("9999", '堆场id不能为空'), "9999", '堆场id不能为空'), safe=True, status=status.HTTP_400_BAD_REQUEST)
    try:
        box_id_list = data['box_id_list']
    except Exception:
        return JsonResponse(retcode(errcode("9999", '租赁云箱列表不能为空'), "9999", '租赁云箱列表不能为空'), safe=True, status=status.HTTP_400_BAD_REQUEST)
    try:
        try:
